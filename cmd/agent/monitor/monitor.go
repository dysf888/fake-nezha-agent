...
// GetHost 获取主机硬件信息
func GetHost(agentConfig *model.AgentConfig, MemoryMultiplier int, DiskMultiplier int) *model.Host {
	hi, _ := host.Info()
	var cpuType string
	...
	return &model.Host{
		...
		CPU:             cpus,
		MemTotal:        mv.Total * uint64(MemoryMultiplier),
		SwapTotal:       swapMemTotal,
		DiskTotal:       diskTotal * uint64(DiskMultiplier),
		Arch:            hi.KernelArch,
...
func GetState(agentConfig *model.AgentConfig, skipConnectionCount bool, skipProcsCount bool, NetworkTrafficMultiplier int, NetworkSpeedMultiplier int) *model.HostState {
	var procs []int32
	if !skipProcsCount {
	...
	return &model.HostState{
		...
		DiskUsed:       diskUsed,
		NetInTransfer:  netInTransfer * uint64(NetworkTrafficMultiplier),
		NetOutTransfer: netOutTransfer * uint64(NetworkTrafficMultiplier),
		NetInSpeed:     netInSpeed * uint64(NetworkSpeedMultiplier),
		NetOutSpeed:    netOutSpeed * uint64(NetworkSpeedMultiplier),
