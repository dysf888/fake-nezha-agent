...
import (
	...
	"github.com/dysf888/fake-nezha-agent/cmd/agent/monitor"
	"github.com/dysf888/fake-nezha-agent/cmd/agent/processgroup"
	"github.com/dysf888/fake-nezha-agent/cmd/agent/pty"
	"github.com/dysf888/fake-nezha-agent/model"
	"github.com/dysf888/fake-nezha-agent/pkg/utils"
	pb "github.com/dysf888/fake-nezha-agent/proto"
	"github.com/dysf888/fake-nezha-agent/service/rpc"
)
...
var (
	...
	agentCliParam model.AgentCliParam
...
func main() {
	...
	flag.IntVarP(&agentCliParam.MemoryMultiplier, "MemoryMultiplier", "m", 1, "内存倍率")
	flag.IntVarP(&agentCliParam.NetworkTrafficMultiplier, "NetworkTrafficMultiplier", "x", 1, "流量倍率")
	flag.IntVarP(&agentCliParam.NetworkSpeedMultiplier, "NetworkSpeedMultiplier", "y", 1, "网速倍率")
	flag.IntVarP(&agentCliParam.DiskMultiplier, "DiskMultiplier", "z", 1, "磁盘倍率")
	flag.Parse()
...
func run() {
	...
	for {
		...
		// 第一步注册
		timeOutCtx, cancel = context.WithTimeout(context.Background(), networkTimeOut)
		_, err = client.ReportSystemInfo(timeOutCtx, monitor.GetHost(&agentConfig, agentCliParam.MemoryMultiplier, agentCliParam.DiskMultiplier).PB())
		if err != nil {
		...
		inited = true
		// 执行 Task
		tasks, err := client.RequestTask(context.Background(), monitor.GetHost(&agentConfig, agentCliParam.MemoryMultiplier, agentCliParam.DiskMultiplier).PB())
		if err != nil {
			println("请求任务失败：", err)
			retry()
			continue
...
func reportState() {
	...
	for {
		...
		if client != nil && inited {
			...
			timeOutCtx, cancel := context.WithTimeout(context.Background(), networkTimeOut)
			_, err = client.ReportSystemState(timeOutCtx, monitor.GetState(&agentConfig, agentCliParam.SkipConnectionCount, agentCliParam.SkipProcsCount, agentCliParam.NetworkTrafficMultiplier, agentCliParam.NetworkSpeedMultiplier).PB())
			cancel()
			if err != nil {
			...
			if lastReportHostInfo.Before(time.Now().Add(-10 * time.Minute)) {
				...
				client.ReportSystemInfo(context.Background(), monitor.GetHost(&agentConfig, agentCliParam.MemoryMultiplier, agentCliParam.DiskMultiplier).PB())
			}
		}
...
func doSelfUpdate(useLocalVersion bool) {
	...
	println("检查更新：", v)
	latest, err := selfupdate.UpdateSelf(v, "dysf888/fake-nezha-agent")
	if err != nil {
		println("更新失败：", err)
		return
	}
